...
% - read prepended json files
% - use multiwaitbar instead of waitbar
    ...
    % remove jsonp padding; ex.: padding({'data':1}) -> {'data':1}
    json_start = regexp(string,'{','once');
    json_end = regexp(string,'}');
    json_end = json_end(length(json_end));
...
%         opt.progressbar_=waitbar(0,'loading ...');
        opt.progressbar_ = multiWaitbar('Reading JSON data',0);
    end
...
%         close(opt.progressbar_);
        multiWaitbar('Reading JSON data','Close');
    end
end
...
%                                         waitbar(pos/length(inStr),pbar,'loading ...');
                                        multiWaitbar('Reading JSON data','Increment',pos/length(inStr))
                                    end
                                    return;
                                end
                            end
...
%         waitbar(pos/len,varargin{1}.progressbar_,'loading ...');
        multiWaitbar('Reading JSON data','Increment',pos/len);
    end
